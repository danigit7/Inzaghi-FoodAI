  ...
  const sendMessage = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Server status: ${response.status} ${response.statusText}`,
          );
        }
    ...
    <div className="w-full max-w-3xl h-[85vh] flex flex-col glass-panel rounded-3xl shadow-2xl overflow-hidden relative animate-fade-in">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 flex items-center gap-1 font-body">
              ...
              Peshawar Foodie AI{" "}
              <span className="text-[9px] opacity-50 ml-1">v1.1</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent relative">
        ...
        {messages.map((msg) => (
          ...
          >
            <div
              className={`flex max-w-[85%] ${msg.sender === "user" ? "flex-row-reverse" : "flex-row"} gap-3`}
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-1 shadow-lg overflow-hidden
                ${
                  msg.sender === "user"
              ...
              <div
                className={`p-4 rounded-2xl shadow-md backdrop-blur-sm transition-all duration-300 hover:shadow-lg ${
                  msg.sender === "user"
                ...
                {msg.suggestions && msg.suggestions.length > 0 && (
                  <div className="mt-4 grid gap-3">
                    {msg.suggestions.map((rest) => (
                      <div
                        key={rest.id}
                        className="bg-[#151722] p-4 rounded-xl border border-[rgba(255,255,255,0.05)] hover:border-blue-500/50 transition-colors group"
                      >
                        ...
                        <div className="mt-3 flex flex-wrap gap-1">
                          {rest.cuisine.slice(0, 3).map((c, idx) => (
                            <span
                              key={idx}
                              className="px-2 py-0.5 bg-[#2a2f45] text-gray-300 rounded-full text-[10px] border border-[rgba(255,255,255,0.05)]"
                            >
                              {c}
                            </span>
                          ))}
      ...
      <div className="p-6 bg-gradient-to-t from-[#0f111a] to-transparent">
        ...
        <p className="text-center text-[10px] text-gray-600 mt-2"></p>
      </div>
    </div>
  );
};
